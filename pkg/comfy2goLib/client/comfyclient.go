...
type ComfyClientOptions struct {
	...
	Auth           *Auth
	ObjectInfoJSON string // 新增：预加载的 object_info JSON 缓存
}
...
type ComfyClient struct {
	...
	httpProtocol          string
	auth                  *Auth
	objectInfoJSONCache   string                // 新增：用于存储来自 Options 的原始 JSON
	objectInfo            *graphapi.NodeObjects // 新增：用于缓存已解析的 object_info
}
...
func NewComfyClientWithOptions(server_address string, server_port int, callbacks *ComfyClientCallbacks, options *ComfyClientOptions) *ComfyClient {
	...
	if options != nil {
		retv.objectInfoJSONCache = options.ObjectInfoJSON
	}
